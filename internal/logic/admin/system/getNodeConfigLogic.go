...
func (l *GetNodeConfigLogic) GetNodeConfig() (*types.NodeConfig, error) {
	...
	if dbConfig.RoutingRules != "" {
		var routingRules []types.NodeRoutingRule
		err = json.Unmarshal([]byte(dbConfig.RoutingRules), &routingRules)
		if err != nil {
			logger.Errorf("[Node] Unmarshal RoutingRules config error: %s", err.Error())
			panic(err)
		}
		c.RoutingRules = routingRules
	}
	if dbConfig.RelayRules != "" {
		var relayRules []types.NodeRelayRule
		err = json.Unmarshal([]byte(dbConfig.RelayRules), &relayRules)
		if err != nil {
			logger.Errorf("[Node] Unmarshal RelayRules config error: %s", err.Error())
			panic(err)
		}
		c.RelayRules = relayRules
	}
