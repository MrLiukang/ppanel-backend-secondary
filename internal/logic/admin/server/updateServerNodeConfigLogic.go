...
func (l *UpdateServerNodeConfigLogic) UpdateServerNodeConfig(req *types.UpdateServerNodeConfigRequest) error {
	...
	effective := nodeconfig.GlobalValues(l.svcCtx.Config.Node)
	if err := nodeconfig.ApplyOverride(&effective, data); err != nil {
		l.Errorf("[UpdateServerNodeConfig] ApplyOverride Error: %v", err.Error())
		return errors.Wrapf(xerr.NewErrCodeMsg(xerr.InvalidParams, "server node config is invalid"), "server node config is invalid: %v", err)
	}
	if err := nodeconfig.ValidateRoutingRules(effective.RoutingRules, effective.Outbound); err != nil {
		l.Errorf("[UpdateServerNodeConfig] ValidateRoutingRules Error: %v", err.Error())
		return errors.Wrapf(xerr.NewErrCodeMsg(xerr.InvalidParams, "routing rules are invalid"), "routing rules are invalid: %v", err)
	}
	if err := nodeconfig.ValidateRelayRules(effective.RelayRules); err != nil {
		l.Errorf("[UpdateServerNodeConfig] ValidateRelayRules Error: %v", err.Error())
