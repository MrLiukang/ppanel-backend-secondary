...
func (l *FilterServerListLogic) FilterServerList(req *types.FilterServerListRequest) (resp *types.FilterServerListResponse, err error) {
	...
	for _, datum := range data {
		...
		override, err := nodeStore.FindServerConfigOverride(l.ctx, datum.Id)
		if err != nil {
			l.Errorf("[FilterServerList] FindServerConfigOverride Error: %s", err.Error())
			return nil, errors.Wrapf(xerr.NewErrCode(xerr.DatabaseQueryError), "FindServerConfigOverride Error")
		}
		if override != nil {
			if err = nodeconfig.ApplyOverride(&nodeValues, override); err != nil {
				l.Errorf("[FilterServerList] ApplyOverride Error: %s", err.Error())
				return nil, errors.Wrapf(xerr.NewErrCode(xerr.DatabaseQueryError), "Apply server node config override error")
			}
		}
		server.RelayRules = nodeconfig.NodeFacingRelayRules(nodeValues.RelayRules)
