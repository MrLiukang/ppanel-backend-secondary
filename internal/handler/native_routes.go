...
func RegisterNativeHandlers(router *server.Hertz, serverCtx *svc.ServiceContext) {
	...
	router.GET("/v2/server/:server_id/relay-subscription-groups", serverHandler.QueryRelaySubscriptionGroupsHandler(serverCtx))
	router.POST("/v2/server/:server_id/relay-subscription-groups/health", serverHandler.ReportRelaySubscriptionGroupHealthHandler(serverCtx))
